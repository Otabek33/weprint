...
from apps.tg.utils import (get_or_create_client, get_or_create_order,
                           ...
                           update_order_file_path, update_order_file_status,
                           update_order_location_sentence,
                           update_order_location_telegram_share,
                           update_order_page_number, update_order_price,
                           update_order_size)
...
def callback_query(call):
    ...
    if call.data == "WHITE" or call.data == "COLOURFUL":
        ...
        bot.send_message(
            ...
            "Qaysi o'lchamda chop etmoqchisiz",
            reply_markup=order_size(),
        )
        ...
        bot.send_message(
            call.message.chat.id,
            "Pereplyot shaklidini tanlang",
            reply_markup=order_binding(),
        )
    elif call.data == "photoOfBinding":
        for binding in bindings:
            bot.send_photo(
                call.message.chat.id, photo=binding.photo, caption=binding.name
            )
        bot.send_message(
            call.message.chat.id,
            "Pereplyot shaklidini tanlang",
        ...
        markup = main_menu()
        bot.send_message(
            call.message.chat.id, "Xizmatlardan birini tanlang", reply_markup=markup
        )
        ...
        bot.send_message(
            call.message.chat.id,
            "Qaysi rangda chop etmoqchisiz 🖨️ 📄?",
            reply_markup=order_color(),
        )
...
def get_sms(message):
    ...
    if message.chat.id == GROUP_CHAT_ID:
        ...
        if user_phone_sharing:
            bot.send_message(
                message.chat.id,
                "Xizmatdan foydalanish uchun telefon raqamingizni yuboring",
            )
        else:
            text = message.text
            if text == "Buyurtma berish 🛒":
                ...
                if text.isnumeric():
                    ...
                    bot.send_message(
                        message.chat.id, "Iltimos sahifalar miqdorini yuboring"
                    )
                ...
                markup = main_menu()
                bot.send_message(
                    message.chat.id, "Xizmatlardan birini tanlang", reply_markup=markup
                )
...
def get_document(message):
    ...
    if message.document.file_size > max_file_size_bytes:
        bot.send_message(
            message.chat.id,
            "Yuborilayotgan hujjat hajmi 50 mb dan kichik bo'lishi kerak",
        )
    else:
        if message.document.file_name.lower().endswith(
                ".pdf"
        ) or message.document.file_name.lower().endswith((".doc", ".docx")):
            ...
            bot.send_message(
                message.chat.id,
                "Xodimimiz tez orada siz bilan bog'lanadi",
            ...
            mess = (
                f"<b>Sizning buyurtmangiz muvoffaqiyatli yaratildi.\n\nTez orada xodimimiz siz bilan bog'lanadi!!!</b>\n\n\n"
            ...
            bot.send_message(
                message.chat.id,
                "Yuborilayotgan hujjat pdf yoki word ko'rinishida bo'lishi kerak",
            )
